 ...
 *
 * Allows users to share product links on social media platforms
 * Supports WhatsApp, Facebook, Twitter, and native Web Share API
 * Uses OG tags endpoint for better social media previews
 */
const ShareButton: React.FC<ShareButtonProps> = ({ product }) => {
  const [showMenu, setShowMenu] = useState(false);

  // Use OG tags endpoint for social media sharing (better previews)
  const ogUrl = `${config.apiBaseUrl}/og/products/${product._id}`;
  const shareText = `Check out ${product.name} at Pattinambakkam Fish World - ₹${product.price}`;
  ...
  return (
    <div className="relative">
      ...
      {/* Share Menu (shows if Web Share API not available) */}
